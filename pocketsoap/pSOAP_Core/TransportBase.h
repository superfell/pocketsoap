...
#pragma comment(lib,"xmltok_static.lib")
...
public:
	HRESULT ConstructSocketInfo(const char * server, UINT port, sockaddr_in &socket )
	{
		...
		else
		{
			...
			if ( !addr )
				return AtlReportError ( static_cast<T *>(this)->GetObjectCLSID(), OLESTR("Failed to resolve server name / IP address"), __uuidof(ISOAPTransport), HRESULT_FROM_WIN32(WSAGetLastError()) );
	...
	static HRESULT Ole2Utf8 ( BSTR &src, stringBuff_A &utf8 ) 
	{
		size_t envLen = SysStringLen(src) ;
		utf8.Allocate(envLen) ;

		// chunk over the source, doing a Utf8 Encode
		int numEncChars ;
		char encBuff[XML_UTF8_ENCODE_MAX] ;
		for ( unsigned long srcIdx = 0 ; srcIdx < envLen ; ++srcIdx )
		{
			numEncChars = XmlUtf8Encode ( src[srcIdx], encBuff ) ;
			utf8.Append( encBuff, numEncChars ) ;
		}
		return S_OK ;
	}
