...
typedef std::multimap<std::string, std::string, ciStringCompare> Headers ;
...
public:
	...
	static bool IsLWS( const wchar_t * c )
	{
		return ( *c == ' ' || *c == '\t' ) ;
	}
	...
	static bool IsIgnoreableChar ( const wchar_t * c )
	{
		...
		while ( p )
		{
			...
			if ( p )
			{
				...
				if ( p ) 
				{
					...
					res.Append ( payload + offset , eoh-p ) ;
					p = eoh ;
				}
			}
		}
